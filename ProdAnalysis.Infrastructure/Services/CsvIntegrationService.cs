...
public sealed class CsvIntegrationService : ICsvIntegrationService
{
    ...
    public async Task<byte[]> ExportDailySummaryCsvAsync(DateOnly from, DateOnly to, Guid? workCenterId)
    {
        if (to < from)
            (from, to) = (to, from);
        ...
        if (workCenterId.HasValue)
        {
            var wc = workCenterId.Value;
            q = q.Where(x => x.WorkCenterId == wc);
        }
        ...
        var evAgg = await db.DeviationEvents
            .AsNoTracking()
            .Where(x => ids.Contains(x.ProductionDayId))
            .GroupBy(x => x.ProductionDayId)
            .Select(g => new
            {
                ProductionDayId = g.Key,
                Total = g.Count(),
                Open = g.Count(x => x.Status != DeviationEventStatus.Closed),
                MaxLevel = g.Max(x => x.CurrentEscalationLevel)
            })
            .ToListAsync();

        var evMap = evAgg.ToDictionary(x => x.ProductionDayId, x => x);

        var sb = new StringBuilder();

        sb.AppendLine("ProductionDayId,Date,WorkCenter,Product,TaktSec,PlanShift,ActualShift,DeviationShift,DowntimeMinutes,DeviationEventsTotal,DeviationEventsOpen,MaxEscalationLevel");

        foreach (var pd in days)
        {
            var plan = pd.HourlyRecords.Sum(h => h.PlanQty);
            var actual = pd.HourlyRecords.Sum(h => h.ActualQty ?? 0);
            var downtime = pd.HourlyRecords.Sum(h => h.HourlyDowntimes.Sum(d => d.Minutes));
            var dev = actual - plan;
            ...
            if (evMap.TryGetValue(pd.Id, out var a))
            {
                totalEv = a.Total;
                openEv = a.Open;
                maxLvl = a.MaxLevel;
            ...
            sb.AppendLine(string.Join(",",
                Esc(pd.Id.ToString()),
                Esc(pd.Date.ToString("yyyy-MM-dd")),
                Esc(pd.WorkCenter.Name),
                Esc(pd.Product.Name),
                pd.TaktSec.ToString(),
                plan.ToString(),
                actual.ToString(),
